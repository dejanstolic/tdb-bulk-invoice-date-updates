...
public class CustomDataCollection<T> implements Serializable{
     ...
     */
    public ParseMessageDto processData(String[] data, Integer row){
        ...
        for (Field field : fields){
            if(field.isAnnotationPresent(CsvBindByPosition.class)){
                ...
                ParseObjectMessage<?> pom = Utils.parseObjectFromString(data[orderId], field.getType(), row, orderId, required);
                if (pom.isValid()) {
...
//                        ParseObjectMessage<?> pomFloat = Utils.parseObjectFromString(number.toString(), field.getType(), row);
...
//                    }
                    int rejected = row+1;
                    ProcessCrossCharges.rejectedList.add(rejected);
                    isValid = false;
                }
